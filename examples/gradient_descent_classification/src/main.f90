...
program p_rectangle_circle 
    ...
    real(real32) :: results(TESTING_SIZE)
    ...
    call n_init(n, WIDTH * HEIGHT, CLASSIFICATION)
    ...
    ! Train the neuron
    print *, "[INFO] Training neuron"
    call n_train(n, train_inputs, train_outputs, 0.001, 100000)
    ...
    print *, "[INFO] Testing neuron"
    do i = 1, TESTING_SIZE
       call n_test(n, test_inputs(i, :), results(i))
    end do
    print *, "[INFO] Perceptron tested"

    sum = 0
    do i = 1, TESTING_SIZE
       ! Mesure the error with the absolute value
        if (abs(results(i) - test_outputs(i)) < 0.5) then
           sum = sum + 1
        else
            print *, "[INFO] test fail ", files_name(i), results(i), test_outputs(i)
        end if
    end do

    ! Print the results
    print *, (sum / real(TESTING_SIZE)) * 100.0, sum, TESTING_SIZE
    print *, "mse: ", mse(n, test_inputs, test_outputs)
...
contains
    ...
    ! load_brain: To load the brain of the neuron in a file
    subroutine load_brain(n)
        type(neuron), intent(in out) :: n
        ...
        ! Catch the weights
        n%w(1:) = reshape(brain, (/WIDTH * HEIGHT/))
        ! Finally read the bias
        read(10) n%w(1)
        close(10)
    end subroutine load_brain

    ! save_brain: To save the brain of the neuron in a file
    subroutine save_brain(n)
        type(neuron), intent(in) :: n
        real(real32) :: brain(HEIGHT, WIDTH)

        ! Catch the matrix
        brain = reshape(n%w(1:), (/HEIGHT,  WIDTH/))
